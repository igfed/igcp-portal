({
        ...
        for (var nIt = 0; nIt < backButtons.length; ++nIt) {
            backButtons[nIt].addEventListener('click', function (evt) {
                ...
                if (component.get('v.loadedByTaxLoader')) {
                    ...
                    $o.navigateToURL(sessionStorage.getItem('previous'), JSON.parse(sessionStorage.getItem('previousStorage')), true);
                }
            });
        }
    ...
    createChatterStuff: function(component, originId) {
        $A.createComponent(
            ...
            function (newCmp, status, errorMessage) {
                if (status === "SUCCESS") {
                    component.set("v.feedPublisher", newCmp);
                }
        ...
        $A.createComponent(
            ...
            function (newCmp, status, errorMessage) {
                if (status === "SUCCESS") {
                    component.set("v.feedViewer", newCmp);
                }
                else if (status === "INCOMPLETE") {
                    console.log("No response from server or client is offline.");
                    // Show offline error
